...
function nextRunPlan(profile: RunnerProfile, dashboard: TrainingDashboard) {
  const goalKm = profile.weeklyDistanceGoalKm || 30;
  const completedKm = (dashboard.weeks[0]?.distanceM || 0) / 1000;
  const remainingKm = Math.max(0, goalKm - completedKm);
  const notes = [
    remainingKm > 0
      ? `เลือกวิ่งสบายประมาณ ${Math.max(1, Math.ceil(remainingKm)).toLocaleString("th-TH")} กม. เพื่อเติมเป้าหมายสัปดาห์นี้`
      : "เป้าหมายระยะทางครบแล้ว เลือกวิ่งเบา ๆ หรือพักฟื้นตามความพร้อมของร่างกาย",
  ];
  if (typeof profile.targetPaceSecondsPerKm === "number") notes.push(`Pace เป้าหมายของคุณคือ ${paceLabel(profile.targetPaceSecondsPerKm)} — ใช้เป็นแนวทางในวันที่พร้อมทำความเร็ว`);
  notes.push("เริ่มต้นช้า ๆ และหยุดหากรู้สึกไม่สบายหรือผิดปกติ");
  return notes;
}
...
export default function Dashboard() {
  ...
  return <main className="live-dashboard">
    ...
    {!ready ? null : !user ? <form className="login-form recent" onSubmit={login}><h2>เข้าสู่ระบบ</h2><label>อีเมล<input type="email" value={email} onChange={e => setEmail(e.target.value)} required /></label><label>รหัสผ่าน<input type="password" value={password} onChange={e => setPassword(e.target.value)} required /></label><button className="upload" disabled={busy}>เข้าสู่ระบบ</button><button type="button" onClick={resetPassword} disabled={busy}>ลืมรหัสผ่าน</button></form> : <>
      ...
      {dashboard && <section className="recent next-run"><h2>แผนวิ่งครั้งถัดไป</h2><p>สรุปจากเป้าหมายและประวัติที่มีอยู่ เป็นแนวทางทั่วไป ไม่ใช่คำแนะนำทางการแพทย์</p><ul>{nextRunPlan(profile, dashboard).map(note => <li key={note}>{note}</li>)}</ul></section>}
