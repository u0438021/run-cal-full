...
export const api = {
  ...
  startAdminTransfer: (toUserId: string, pin: string) => call<{ transferId: string; expiresInMinutes: number }>('startAdminTransfer', { toUserId, pin }),
  acceptAdminTransfer: (transferId: string) => call<{ accepted: boolean }>('acceptAdminTransfer', { transferId }),
  markRead: (notificationId: string) => call<{ read: boolean }>('markNotificationRead', { notificationId }),
  fit: async (file: File) => {
